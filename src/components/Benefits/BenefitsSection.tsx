...
const BenefitCard = ({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) => {
  ...
  return (
    <div 
      ref={cardRef}
      className={`section-animate scale-in ${isVisible ? 'appear' : ''} bg-white p-6 rounded-2xl shadow-lg transition-transform duration-300 hover:transform hover:scale-105`}
    >
...
export const BenefitsSection = () => {
  ...
  return (
    <section 
      ref={sectionRef}
      id="benefits" 
      className={`py-20 bg-gradient-to-br from-blue-50 to-indigo-50 section-animate from-right ${isSectionVisible ? 'appear' : ''}`}
    >
